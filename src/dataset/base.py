...
class BaseUnpairedDataset(torch.utils.data.Dataset, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        self.dataset = (
            dataset
                .map(self.preprocess_fn, num_proc=num_proc)
                .map(self.apply_template, num_proc=num_proc)
        )
    ...
    def apply_template(self, example: dict[str, any]) -> dict[str, str]:
        ...
        max_prompt_length = min(self.max_prompt_length, len(prompt["input_ids"]))
        ...
        return {
            ...
            "prompt_attention_mask": prompt["attention_mask"][-max_prompt_length:],
            "chosen_input_ids": chosen["input_ids"][:max_response_length],
            "chosen_attention_mask": chosen["attention_mask"][:max_response_length],
        }
    ...
    def collate_fn(self, batch: list[dict[str, any]]) -> dict[str, torch.Tensor]:
        ...
        """
        max_length = max(len(example["prompt_input_ids"] + example["chosen_input_ids"]) for example in batch)
        # pad chosen
        self.tokenizer.padding_side = "right"
        chosen = self.tokenizer.pad(
            encoded_inputs={
                "input_ids": [example["prompt_input_ids"] + example["chosen_input_ids"] for example in batch],
                "attention_mask": [example["prompt_input_ids"] + example["chosen_attention_mask"] for example in batch],
            },
            padding="max_length",
        ...
        chosen_labels = pad_sequence(
            sequences=[
                torch.tensor(
                    len(example["prompt_attention_mask"]) * [-100] + example["chosen_input_ids"],
        ...
        chosen_labels = torch.cat(
            (
                ...
                chosen_labels.new_full(
                    (chosen_labels.shape[0], max_length - chosen_labels.shape[1]), -100
                )
            ),
            dim=1
        )
...
class BasePairedDataset(BaseUnpairedDataset, metaclass=abc.ABCMeta):
    ...
    def apply_template(self, example):
        ...
        return {
            ...
            "rejected_attention_mask": rejected["attention_mask"][:max_response_length],
        }
    
    def collate_fn(self, batch: list[dict[str, any]]) -> dict[str, torch.Tensor]:
        # pad prompt from left
        max_length = max(
            max(len(example["prompt_input_ids"] + example["chosen_input_ids"]) for example in batch),
            max(len(example["prompt_input_ids"] + example["rejected_input_ids"]) for example in batch),
        )
        ...
        chosen = self.tokenizer.pad(
            encoded_inputs={
                "input_ids": [
                    example["prompt_input_ids"] + example["chosen_input_ids"] for example in batch
                ],
                "attention_mask": [
                    example["prompt_attention_mask"] + example["chosen_attention_mask"]
                    for example in batch
                ],
            },
            max_length=max_length,
        ...
        chosen_labels = torch.cat(
            (
                chosen_labels,
                chosen_labels.new_full(
                    (chosen_labels.shape[0], max_length - chosen_labels.shape[1]),
                    fill_value=-100,
                ),
            ),
            dim=1,
        ...
        rejected = self.tokenizer.pad(
            encoded_inputs={
                "input_ids": [
                    example["prompt_input_ids"] + example["rejected_input_ids"] for example in batch
                ],
                "attention_mask": [
                    example["prompt_attention_mask"] + example["rejected_attention_mask"]
                    for example in batch
                ],
            },
            max_length=max_length,
        ...
        rejected_labels = pad_sequence(
            sequences=[
                torch.tensor(
                    len(example["prompt_attention_mask"]) * [-100] + example["rejected_input_ids"]
                )
                for example in batch
            ],
            batch_first=True,
        ...
        rejected_labels = torch.cat(
            (
                ...
                rejected_labels.new_full(
                    (rejected_labels.shape[0], max_length - rejected_labels.shape[1]),
                    fill_value=-100,
                ),
            ),
            dim=1,
