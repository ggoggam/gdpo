...
from algo.mixins import InstantiateDatasetMixin, InstantiateModelMixin


class OfflineTrainer(BaseTrainer, InstantiateModelMixin, InstantiateDatasetMixin):
    ...
    def train(self):
        ...
        for epoch in range(self.config.epochs):
            ...
            for batch in self.train_loader:
                ...
                self.set_mode(is_train=True)
                with self.accelerator.autocast(AutocastKwargs(self.model.training and self.config.mixed_precision == "fp16")):
                    with self.accelerator.accumulate(self.model):
                        ...
                        if self.accelerator.sync_gradients:
                            ...
                            progress.set_description(
                                f"Epoch: [{epoch + 1}/{self.config.epochs}] Steps: [{self.batches_seen}/{total_steps}] (loss {mean_loss:.4f})"
                            )
                            self.update_steps += 1
                            self.logger.log(
                                {
