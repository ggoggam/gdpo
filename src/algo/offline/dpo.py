...
class DPOTrainer(OfflineTrainer):
    def __init__(self, *args, **kwargs):
        ...
        self.ref_model = self._instantiate_model(AutoModelForCausalLM)
    ...
    def _concat_forward(
        ...
        logps, mask = self.compute_token_logps(logits=logits, labels=concat_labels)
        # reduce logps
        if reduce == "mean":
    ...
    def loss(
        ...
        (*_, policy_chosen_logps, policy_rejected_logps,) = self._concat_forward(
            model=self.model,
        ...
        with torch.no_grad():
            (*_, ref_chosen_logps, ref_rejected_logps,) = self._concat_forward(
        ...
        if self.config.algorithm.loss_type == "ipo":
            loss = (logits - 1 / (2 * self.beta)) ** 2
        elif self.config.algorithm.loss_type == "sigmoid":
            loss = (
                -F.logsigmoid(self.beta * logits)
                * (1 - self.config.algorithm.label_smoothing)
                - F.logsigmoid(-self.beta * logits)
                * self.config.algorithm.label_smoothing
            )
        elif self.config.algorithm.loss_type == "normcdf":
            loss = -torch.log(
                0.5
                * (1 + torch.erf(self.config.algorithm.beta * logits / math.sqrt(2)))
            )
        else:
            raise ValueError(f"Unknown loss type: {self.config.algorithm.loss_type}")

        chosen_rewards = (
            self.config.algorithm.beta
            * (policy_chosen_logps.sum(dim=-1) - ref_chosen_logps.sum(dim=-1)).detach()
        )
        rejected_rewards = (
            self.config.algorithm.beta
            * (
                policy_rejected_logps.sum(dim=-1) - ref_rejected_logps.sum(dim=-1)
            ).detach()
        )
