...
class GDPOTrainer(OfflineTrainer, InstantiateModelMixinWithReferenceModel):
    ...
    def loss(
        ...
        with torch.no_grad():
            ...
            chosen_rewards, rejected_rewards = log_rewards.chunk(
                chunks=2, dim=0
            )
        ...
        detailed_balance = (
            log_flows[:, :-1] - log_flows[:, 1:] + policy_logps[:, :-1]
        )
        detailed_balance = (
            (detailed_balance * mask[:, :-1]).pow(2).sum(dim=-1).mean()
        )
        ...
        metrics = {
            ...
            "{split}/rewards": (
                log_rewards.sum(dim=-1) / mask.sum(dim=-1)
            ).mean(),
