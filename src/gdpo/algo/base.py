...
from gdpo.config import TrainConfig
from gdpo.logger import MultipleLogger, RichLogger, WandbLogger
from gdpo.mixin import ComputeMixin, GatherMetricsMixin, ReleaseMemoryMixin
...
class BaseTrainer(abc.ABC, ComputeMixin, GatherMetricsMixin, ReleaseMemoryMixin):
    ...
    def save_model(
        self, name: str | None = None, metrics: dict[str, float] | None = None,
    ) -> None:
        ...
        if hasattr(unwrapped, "_orig_mod"):
            unwrapped = unwrapped._orig_mod  # noqa: SLF001
        ...
        if metrics and self.accelerator.is_main_process:
            # dump metrics
            with (path / "metrics.json").open("w") as f:
                json.dump(metrics, f, indent=2)

        # dump training config
        with (path / "train_config.json").open("w") as f:
    ...
    def load_checkpoint(self, path: os.PathLike | None = None) -> None:
        ...
        if self.accelerator.is_main_process:
            with (path / "trainer_state.json").open() as f:
                state = json.load(f)
