...
from gdpo.algo.base import BaseTrainer
from gdpo.mixin import InstantiateTrainerDatasetMixin
...
class OfflineTrainer(BaseTrainer, InstantiateTrainerDatasetMixin):
    ...
    def _initialize_acceleration(self):
        ...
        self.model, *other_wrapped = self.accelerator.prepare(
            *(getattr(self, x) for x in wrappable_attributes),
        )
        for attr, wrapped in zip(wrappable_attributes[1:], other_wrapped, strict=False):
            ...
            for batch in self.train_loader:
                if self.batches_seen % self.config.eval_every == 0 and (
                    ...
                    if self.batches_seen > 0:
                        self.save_checkpoint(f"step-{self.batches_seen}", eval_metrics)

                start_time = time.time()

                self.set_mode(is_train=True)
                is_fp16 = (
                    self.model.training and self.config.mixed_precision == "fp16"
                )
                with (
                    self.accelerator.autocast(AutocastKwargs(is_fp16)),
                    self.accelerator.accumulate(self.model),
                        ...
                        num_examples = self.gather_metrics(
                            torch.tensor(num_examples, device=self.accelerator.device),
                        ...
                        if self.accelerator.sync_gradients:
                            ...
                            mean_loss = mean_metrics.get("train/loss", float("nan"))

                            progress.set_description(
                                f"[Epoch {epoch + 1}/{self.config.epochs}]"
                                f" [Step {self.batches_seen}/{total_steps}]"
                                f" (loss {mean_loss:.4f})",
                            )
                            self.update_steps += 1
                            ...
                            if self.batches_seen % self.config.log_every == 0:
                                mean_metrics.update(
                                    {
                                        "counter/lr": self.scheduler.get_last_lr()[0],
                                ...
                                self.logger.log(mean_metrics, step=self.examples_seen)
                                accumulated.clear()
    ...
    def evaluate(self) -> dict[str, Any]:
        ...
        return {
            k: sum(v) / len(v) if len(v) > 0 else 0 for k, v in accumulated.items()
        }
