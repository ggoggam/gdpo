...
from gdpo.algo.offline.base import OfflineTrainer
from gdpo.mixin.instantiate import InstantiateModelMixinWithReferenceModel


class DPOTrainer(OfflineTrainer, InstantiateModelMixinWithReferenceModel):
    reference_model: torch.nn.Module | PreTrainedModel
    ...
    def _concat_forward(
        ...
        """Concatenates chosen and rejected response in batch dim for faster training.
        ...
        Args:
            ...
            reduce (Literal["none", "mean", "sum"], optional):
                reduction in vocab dimension. Defaults to "sum".

        Returns:
            tuple[torch.FloatTensor]:
                tuple of (chosen_logits, rejected_logits, chosen_logps, rejected_logps)
    ...
    def loss(
        ...
        if self.loss_type == "ipo":
            ...
            loss = -torch.log(0.5 * (1 + torch.erf(self.beta * logits / math.sqrt(2))))
        else:
