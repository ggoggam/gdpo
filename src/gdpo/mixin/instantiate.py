...
from transformers import AutoModel, AutoTokenizer, PreTrainedModel, get_scheduler
from transformers.utils import is_flash_attn_2_available

from gdpo.config import SFTConfig
from gdpo.dataset import (
    PairedAnthropicHH,
    PairedTLDR,
    UnpairedAnthropicHH,
    UnpairedTLDR,
)
from gdpo.mixin.base import TrainerMixinProtocol, TrainerWithReferenceModelMixinProtocol
from gdpo.mixin.utilities import TorchDtypeMixin
...
class BaseInstantiateModelMixin(abc.ABC, TorchDtypeMixin):
    ...
    def _instantiate_tokenizer(
        ...
        if tokenizer.chat_template is None:
            with open("config/template/chatml.jinja") as f:  # noqa: PTH123
                tokenizer.chat_template = f.read()
        # some cases require relaxing the template
        elif "[INST]" in tokenizer.chat_template and relax_template:
            with open("config/template/llama.jinja") as f:  # noqa: PTH123
                tokenizer.chat_template = f.read()
        self.tokenizer = tokenizer

    def _instantiate_optimizer(self: TrainerMixinProtocol):
        """Instantiates optimizer from a class.

        Must be called after dataset instantiation.
        """
        self.logger.print(">>> Optimizer Initialization <<<")
        # optimizer
        no_decay_names = [
            n.lower() in {"bias", "layernorm.weight"}
...
class InstantiateTrainerDatasetMixin:
    def _instantiate_dataset(
        ...
        if is_train and self.config.train_proportion < 1.0:
            pct = int(self.config.train_proportion * 100)
            split = f"{dataset_cls.train_split}[:{pct}%]"
        with self.accelerator.main_process_first():
            hf_dataset = datasets.load_dataset(dataset_cls.dataset_name, split=split)
            return dataset_cls(
                dataset=hf_dataset,
