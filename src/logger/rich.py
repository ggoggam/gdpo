...
from logger.base import BaseLogger, main_process_only
...
class RichLogger(BaseLogger):
    ...
    def log(self, metrics: dict[str, any], step: int, **kwargs) -> None:
        joined = ",".join(f"{key}: {value}" for key, value in metrics.items())
        self._writer.print(f"[Step {step}] {joined:<100}")
    ...
    def log_table(self, title: str, data: dict[str, any], **kwargs) -> None:
        ...
        tabular = table.Table(
            title=title, show_lines=True, title_justify="left"
        )
